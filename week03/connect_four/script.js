(function() {
    ...
    $(".column").on("click", function(e) {
        ...
        var slotsInDiagonals = [];
        for (var j = 0; j < diagonalsIndex.length; j++) {
            for (var k = 0; k < diagonalsIndex[j].length; k++) {
                slotsInDiagonals.push($(".slot").eq(diagonalsIndex[j][k]));
            }
        }

        console.log("slots in diagonal: ", slotsInDiagonals);

        if (
            checkForVictory(slotsInColumn) ||
            checkForVictory(slotsInRow) ||
            checkForVictory(slotsInDiagonals)
        ) {
